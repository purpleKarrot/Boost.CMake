...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="format.cvs.revision">
    ...
    <xsl:value-of select="concat($month.name, ' ', $day, ', ', $year, ' at ', $time, ' GMT')"/>
  </xsl:template>
  ...
  <xsl:template name="format.svn.revision">
    ...
    <!-- Get the time -->
    <xsl:variable name="time" select="substring-before($text.noday, ' ')" />
    <xsl:variable name="text.notime" select="substring-after($text.noday, ' ')" />
    ...
    <xsl:value-of select="concat($month.name, ' ', $day, ', ', $year, ' at ', $time, ' ', $timezone)"/>
  </xsl:template>
